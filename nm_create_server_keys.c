...
//-------------------------------------------------------------------------------
int usage(){
	printf("This is the usage statement...\n");
...
int main (int argc, char **argv) {
  ...
  rslt = natmsg_gen_key(buff_offline_sign_sexp,
		&entry_stuff,
		buff_offline_sign_pub_sexp_result, 
		buff_offline_sign_prv_sexp_result,
		MAX_KEY_BUFF,
		&sexp_offline_sign_key, DEBUG_LVL);

	if (rslt){
		perror("Generation of offline signing key failed\n");
		exit(720);
	}

	// Write the PUB key to a file:
	strcpy(output_fname, entry_stuff.output_fname_prefix);
	strcat(output_fname, "OfflinePUBSignKey.key");
	printf("out fname 1 is %s", output_fname);
	if (strlen(buff_offline_sign_pub_sexp_result) > 0){
		fp = fopen((char *) output_fname, "w");
		if(!fp){
			perror("Error. Could not open the Offline Public Signing output file.");
			return(345);
		}
		fprintf(fp,"%s", buff_offline_sign_pub_sexp_result);
		fclose(fp);
		// Write the prv key to a file:
		strcpy(output_fname, entry_stuff.output_fname_prefix);
		strcat(output_fname, "OfflinePRVSignKey.key");
		fp = fopen((char *) output_fname, "w");
		if(!fp){
			perror("Error. Could not open the Offline Private Signing Key output file.");
			return(345);
		}
		fprintf(fp,"%s", buff_offline_sign_prv_sexp_result);
		fclose(fp);
	}else{
		printf("Error.  The offline key was not generated.\n");
	}
	//------------------------------------------------------------
	printf("\n -=-=-=-=-=-=- Starting Online Enc keygen\n");
	strncpy(entry_stuff.name_real, save_name, MAX_ENTRY_LEN);
  ...
  rslt = natmsg_gen_key(buff_online_enc_sexp, 
	...
	strncpy(entry_stuff.name_real, save_name, MAX_ENTRY_LEN );
	strcpy(entry_stuff.key_function, "s"); //signing key
	// Is the LEN here the max for the resulting string or nbr of chars copied?
